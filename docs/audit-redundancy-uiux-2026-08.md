...
**Status.** Fixed, then corrected. The first attempt promoted the two hidden reports to top-level tabs — which fixed the visibility and broke the grouping: Desempeño de Ventas belongs to Ventas and Calidad de Leads to Marketing, and that hierarchy was a decision the client had already taken. The right answer was both: make them visible **inside** their group. The bar now has three groups — Dirección · Ventas · Marketing — each with its own sub-tabs in the app's single sub-tab row, from a declarative table (`NAV_GRUPOS`) that also carries the permissions, so a report can't be moved without its permission moving with it. The Canal and Módulo selectors hang off the sub-tab rather than the group, because Desempeño de Ventas, CRM en vivo and Calidad de Leads read the whole CRM and a selector that picks nothing is worse than none. The URL became `#/grupo/sub`, and the ←/→ buttons now move it too — they were changing the view and leaving the old URL behind, so copying it mid-session gave a link to a different screen.
